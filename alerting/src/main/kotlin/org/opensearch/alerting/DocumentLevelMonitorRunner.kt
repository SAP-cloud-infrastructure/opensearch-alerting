        ...
        try {
            ...
            docLevelMonitorInput.indices.forEach { indexName ->
                ...
                concreteIndices.forEach { concreteIndexName ->
                     ...
                     * shouldCreateSingleAlertForFindings monitors also require single-node fan-out:
                     * the duplicate-alert fix relies on per-concrete-index sequential dispatch
                     * (refresh-then-search) which breaks if multiple nodes process the same index
                     * concurrently and race on alert creation.
                     **/
                    ...
                    val nodeMap: Map<String, DiscoveryNode> = if (docLevelMonitorInput?.fanoutEnabled == true &&
                        monitor.shouldCreateSingleAlertForFindings != true
                    ) {
                        getNodes(monitorCtx)
                    } else {
